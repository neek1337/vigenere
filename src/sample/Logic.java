...
public class Logic {
    ...
    public static HashMap<Integer, ArrayList<String>> dic;
    public static int[] truekey;

    public static String main(String string) throws IOException, InterruptedException {
       ...
       /* Scanner scanner = new Scanner(new File("C:\\Users\\Никита\\Downloads\\vij\\src\\message.txt"));
        ...
        fileWriter.close();;
          */
        dic = new HashMap<Integer, ArrayList<String>>();
        int dicsiz = 100000 / 8;
        Scanner scanner1 = new Scanner(new File("areverse.txt"));
        int number = dicsiz;
        int local = 0;
        while (scanner1.hasNext()) {
            if (number == dicsiz) {
                number = 0;
                dic.put(local, new ArrayList<String>());
                local++;
            }
            dic.get(local - 1).add(scanner1.next());
            number++;
        }
        ...
        for (int i = 0; i < spaces.size() - 1; i++) {
            if (spaces.get(i + 1) - spaces.get(i) > 4) {
                start = spaces.get(i) - i;
                finish = spaces.get(i + 1) - i - 1;
                break;
            }
        }
        ...
        System.out.println(textWOSpaces.substring(start, finish));
        ...
        check(textWOSpaces, distance, maxs, start, finish, service);
    ...
    public static void check(String s1, int distance, int[] maxs, int start, int finish, ExecutorService service) throws InterruptedException {
        List<String> list = val(distance);
        int index = 0;
        while (!founded) {
            String s = s1.substring(0, finish);
            ...
            decoded = decoded.substring(start, decoded.length() - tail);
            for (int i = 0; i < 8; i++) {
